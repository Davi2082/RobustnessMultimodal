...
Usage:
    python3 -m scripts.main_scripts.run_ablation --dataset Recovery --device cuda:0
    python3 -m scripts.main_scripts.run_ablation --devices all
"""
...
def main():
    ...
    print(f"  Device(s): {', '.join(args.devices)}")
    print("=" * 70)
    ...
    if args.force or not os.path.isfile(ff_ablation_csv):
        ...
        run([
            sys.executable, "-m", "scripts.utils.modality_ablation",
