...
from configuration_files.paths import RESULT_PATH, CLEAN_FF_PARAMS, DATA_PERTURBED_FF
...
def main():
    ...
    parameter_parser.add_argument("--parameters-path", default=CLEAN_FF_PARAMS)
    ...
    parser.add_argument("--attack-scope", choices=("image", "text", "both"), default="both")
    parser.add_argument("--optimization", choices=("sum", "alternating"), default="alternating")
    parser.add_argument("--text-attack", choices=("trepat", "hotflip"), default="trepat")
    ...
    bertattack_tokenizer = bertattack_mlm = trepat_rephraser = None
    if args.attack_scope in {"text", "both"}:
        if args.text_attack == "trepat":
            trepat_rephraser = Rephraser(model=ATTACK_MODEL, device=device_mlm, command=COMMAND)
        else:
    ...
    # Results dir setup
    experiment_name = args.experiment_name or f"{args.attack_scope}-{args.text_attack}-{args.optimization}"
    output_dir = os.path.join(args.results_path, "perturbed", "feature-fusion", experiment_name)
    dump_dir = os.path.join("data_perturbed", args.dataset, "feature-fusion", experiment_name)
    ...
    for images, labels, texts, imgs_path, indices in tqdm(dataloader_test, desc="Evaluating batches", total=len(dataloader_test)):
        ...
        for i, label in tqdm(enumerate(labels.tolist()), desc="Attacking samples of the batch", total=len(labels), leave=False):
            ...
            if label == args.source_label:
                news_per = news
                rounds = args.alternation_rounds if args.optimization == "alternating" else 1
                for _ in range(rounds):
                    attack_base = news if args.optimization == "sum" else news_per
                    if args.attack_scope in {"image", "both"}:
                        news_img_per, _, _ = img_perturbation(model, tokenizer, processor, args, attack_base, torch.tensor([label], device=device))
                        news_per = {"txt": news_per["txt"], "img": news_img_per["img"]}
                    if args.attack_scope in {"text", "both"}:
                        text_base = news if args.optimization == "sum" else news_per
                        with torch.no_grad():
                            if args.text_attack == "trepat":
                                news_txt_per, txt_similarity = trepat_attack(model, tokenizer, processor, args, text_base, label, device, trepat_rephraser)
                            else:
                                news_txt_per, txt_similarity = bertattack_attack(model, tokenizer, processor, args, text_base, label, device, bertattack_tokenizer, bertattack_mlm, device_mlm)
                ...
                save_perturbed_image(os.path.join(dump_dir, "images"), indices[i].item(), news_per["img"])
    ...
    save_perturbed_texts(dump_dir, perturbed_text_rows)

    # Save "Parameters" in a file
    attack_parameters = {
        "Source Label": args.source_label,
        "Target Label": args.target_label,
        "Attack Scope": args.attack_scope,
        "Optimization": args.optimization,
        "Text Attack": args.text_attack,
